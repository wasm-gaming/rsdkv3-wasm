// Standalone demo harness for the RSDKv3 SDK.
//
// Compiles to dist/demo.js and is loaded by dist/index.html. It consumes the SDK
// by package name (resolved via the page's import map to ./rsdkv3/rsdkv3.sdk.js),
...
async function boot(source: { data?: Uint8Array }): Promise<void> {
  ...
  try {
    ...
    const engine: Rsdkv3Instance = await load({
      canvas,
      assets: {
...
function wireEscape(engine: Rsdkv3Instance): void {
  let paused = false;
